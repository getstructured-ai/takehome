...
Build an agentic chatbot with tool-calling capabilities.
...
Build an **agentic chatbot** that can have conversations and use tools to accomplish tasks. This will only be for one user so don't worry about building a login flow, auth or security. Do however ensure the processes are async so that in theory if multiple users were on the platform it would not halt. 
